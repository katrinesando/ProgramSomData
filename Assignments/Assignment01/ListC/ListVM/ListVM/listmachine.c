...
int execcode(word p[], word s[], word iargs[], int iargc, int /* boolean */ trace) {
  ...
  for (;;) {
    ...
    case PRINTI:
      printf(WORD_FMT " ", IsInt(s[sp]) ? Untag(s[sp]) : s[sp]); printf("\n");break;
    case PRINTC:
      printf("%c", (char)Untag(s[sp])); printf("\n");break;
    case LDARGS: {
      int i;
    ...
    case CREATETABLE: {
        word size = Untag(s[sp--]);; // Untag index fra stacken
        if (size<0 || !IsInt(size))
        {
          printf("ERROR: Invalid table size WORD_FMT", size);
          return -1;
        }
          
        word* table = allocate(CREATETABLE, size + 2, s, sp);
        table[1] = Tag(size);
        //table[1] = Tag(size);
    ...
    case UPDATETABLE: {
        word value = Untag(s[sp--]); // Untag værdien fra stacken
        word index = Untag(s[sp--]); // Untag index fra stacken
        word* table = (word*)s[sp--]; // Hent tabellen fra stacken og untag den
        word tableSize = Untag(table[1]); // Hent størrelsen af tabellen og untag den
        if (index >= 0 && index <= tableSize+1) {
          table[index+2] = Tag(value); // Opdater tabellen med den nye værdi og tag den
        } else {
...
{
  if(!IsInt(*block) && inHeap(block) && !Color(*block)==Black)
  {
